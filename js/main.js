...
// FAQ accordion behavior (added)
$(document).ready(function(){
  $('.faq_section .question').on('click', function(){
    var $q = $(this);
    var $item = $q.closest('.faq_item');
    var $col = $item.closest('.faq_col');
    var $answer = $q.next('.answer');
    ...
    if(isOpen){
      ...
      // close other open items in same column
      $col.find('.faq_item.open').each(function(){
        $(this).removeClass('open');
        $(this).find('.answer').css('max-height','0');
        $(this).find('.question').attr('aria-expanded','false');
        $(this).find('.icon').text('+');
      });

      $item.addClass('open');
      // set to scrollHeight so transition animates to full height
  ...
  // Ensure any pre-open items get correct height on load
  $('.faq_item.open .answer').each(function(){
